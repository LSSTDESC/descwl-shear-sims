...
def test_shear_halo():
    ...
    assert lensed_shift != input_shift, "Lensed shift should not be the same as input shift"
    assert gamma1 < 0, "Gamma1 should be negative"
    assert gamma2 == 0, "Cross shear should be zero"

    shear_halo_no_kappa = ShearHalo(mass=mass, conc=4, z_lens=z_lens, no_kappa=True)
    gso = galsim.Gaussian(sigma=1)
    input_shift = galsim.PositionD(1, 0)
